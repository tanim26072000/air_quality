...
col1, col2 = st.columns(2)
with col1:
    selected_model_1 = st.selectbox('Select first model', [
        'Observed','GNN+LSTM', 'GNN', 'CNN+LSTM', 'CNN'], key='select_model')
with col2:
    selected_model_2 = st.selectbox('Select second model', [
        'Observed', 'GNN+LSTM', 'GNN', 'CNN+LSTM', 'CNN'], key='select_model_2')
selected_model_2 = selected_model_2.lower()
selected_model_1 = selected_model_1.lower()
...
df = df.assign(observed_status=df[selected_model_1].apply(lambda x: get_status_color(x)[0]),
               observed_color=df[selected_model_1].apply(
                    lambda x: get_status_color(x)[1]),
               observed_color_css=df[selected_model_1].apply(
                    lambda x: get_status_color(x)[2]),
               predicted_status=df[selected_model_2].apply(
                    lambda x: get_status_color(x)[0]),
               predicted_color=df[selected_model_2].apply(
                    lambda x: get_status_color(x)[1]),
               predicted_color_css=df[selected_model_2].apply(
                    lambda x: get_status_color(x)[2]),
            #    observed=df[selected_model_1].round(2),
               predicted_elevation= df[selected_model_2] * 50,
               observed_elevation= df[selected_model_1] * 50
               )
# df[['predicted_elevation', 'observed_elevation']] = df[[selected_model_2,selected_model_1]] * 50
...
if st.button('Generate Plot'):
    ...
    #     get_fill_color=f'[255, {selected_model_2} * 2, 100]',
    #     pickable=True,
    #     tooltip=True
    # )
    observed_layer = pdk.Layer(
    ...
    observed_deck = pdk.Deck(
        ...
        tooltip={
            "html": f"""
                ...
                <b>{selected_model_1}:</b> <span style='color:{{observed_color_css}};'>{{{selected_model_1}}} ({{observed_status}})</span><br/>
                <b>{selected_model_2}:</b> <span style='color:{{predicted_color_css}};'>{{{selected_model_2}}} ({{predicted_status}})</span>
                """,
            "style": {
                "color": "white"
            }
    ...
    with col1:
        st.markdown(f"""
        <h3 style='text-align: center;'>Data Map by {selected_model_1}</h3>
    """, unsafe_allow_html=True)
        st.pydeck_chart(observed_deck)

    with col2:
        st.markdown(f"""
        <h3 style='text-align: center;'>Data Map by {selected_model_2}</h3>
    """, unsafe_allow_html=True)
    ...
    observed_min = df[selected_model_1].min()
    observed_max = df[selected_model_1].max()
    observed_mean = df[selected_model_1].mean()
    observed_median = df[selected_model_1].median()

    predicted_min = df[selected_model_2].min()
    predicted_max = df[selected_model_2].max()
    predicted_mean = df[selected_model_2].mean()
    predicted_median = df[selected_model_2].median()

    # RMSE (Root Mean Squared Error)
    # Assuming 'actual' and 'predicted' are the columns of your dataframe
    df['squared_diff'] = (df[selected_model_1] - df[selected_model_2]) ** 2
    rmse = np.sqrt(df['squared_diff'].mean())

    # MAE (Mean Absolute Error)
    mae = mean_absolute_error(df[selected_model_1], df[selected_model_2])
    ...
    # | Statistic/Category                 | Observed | Predicted ({selected_model_2}) |
    # |-----------------------------------|-----------------------|------------------------|
    ...
    # """, unsafe_allow_html=True)
    # Define a flag to check if 'observed' is selected in either model
    show_metrics = selected_model_1 == 'observed' or selected_model_2 == 'observed'

    # Build the markdown content with conditional inclusion of RMSE and MAE
    markdown_content = f"""
        ### Statistical overview:
        Total data points within selected spatio-temporal range: {total_count}<br>
    """

    # Conditionally add RMSE and MAE if 'observed' is selected in any model
    if show_metrics:
        markdown_content += f"""
        Root of mean squared error (RMSE): {rmse:.2f}<br>
        Mean absolute error (MAE): {mae:.2f}<br>
    ...
    # Add the rest of the markdown content
    markdown_content += f"""
    **Comparison of Observed and Predicted Values of the selected model**

    | Statistic/Category                 | {selected_model_1} | {selected_model_2} |
    |-----------------------------------|-----------------------|------------------------|
    | **Minimum (µg/m³)**               | {observed_min:.2f}    | {predicted_min:.2f}    |
    | **Maximum (µg/m³)**               | {observed_max:.2f}    | {predicted_max:.2f}    |
    | **Mean (µg/m³)**                  | {observed_mean:.2f}   | {predicted_mean:.2f}   |
...
# # Display the metrics
# st.write(f"### Model: {selected_model_2}")
